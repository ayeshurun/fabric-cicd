...
"""Shared file processing for Org App and Org App Audience items.

Both item types reference other items from their definition.json and require the same
reference-resolution logic at publish time. This module holds that shared logic so each
publisher can consume it without importing from the other.
"""
...
def _is_item_reference(value: dict) -> bool:
    """
    Returns True when the value is a reference to another item.

    A reference is identified either by an "elementType" of "item" (Org App layout) or
    by carrying an "itemType" (Org App Audience layout).

    Args:
        value: The value to inspect.
    """
    return value.get("elementType", "") == "item" or bool(value.get("itemType", ""))
